  ...
  setup() {
    ...
    const cursosFiltrados = computed(() => {
      ...
      return cursos.value.filter((curso) => {
        // Verifica si Modo_Curso existe antes de acceder a nombre_modo
        return curso.Modo_Curso && curso.Modo_Curso.nombre_modo === filtros.value.modo;
      });
    });
    ...
    const fetchOperadores = async () => {
      if (!selectedHora.value || !selectedFecha.value || !filtros.value.curso) return;

      try {
        const { data, error } = await supabase
          .from("matriculas")
          .select("id, fk_operador, operador:fk_operador (nombre, apellidos)")
          .eq("fk_curso", filtros.value.curso)  // Filtra por curso
          .eq("fk_sesion",
            sesiones.value.find(
              (sesion) =>
